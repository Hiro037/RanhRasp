...
async def _send_message(user: User, text: Optional[str] = None, image_bytes: Optional[bytes] = None):
    """Отправляет сообщение пользователю на его платформу."""
    try:
        if user.platform == "telegram":
            if image_bytes:
                from aiogram.types import BufferedInputFile
                photo = BufferedInputFile(image_bytes, filename="schedule.png")
                await tg_bot.send_photo(chat_id=user.platform_id, photo=photo, caption=text)
            else:
                await tg_bot.send_message(chat_id=user.platform_id, text=text, parse_mode="Markdown")
            ...
            if image_bytes:
                ...
                await vk_bot.api.messages.send(
                    peer_id=user.platform_id,
                    message=text or "Расписание на сегодня",
        ...
        print(f"Ошибка отправки уведомления пользователю {user.id} ({user.platform}): {e}")
...
async def send_daily_notifications():
    ...
    print(f"[DailyNotifier] Запуск рассылки на {target_date}")
    ...
    tasks = []
    for user in users:
        tasks.append(send_daily_notification_to_user(user, target_date))

    if tasks:
        await asyncio.gather(*tasks)

    print(f"[DailyNotifier] Рассылка завершена. Отправлено {len(tasks)} уведомлений.")
